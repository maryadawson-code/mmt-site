// ============================================================
// jun12-expert-test-capture-send.js — Netlify Scheduled Function
//
// Sends the June 12, 2026 Capture Corner teaser ("The Expert Test") to
// all active premium subscribers. Premium companion to the public
// newsletter "The Expert You Cannot Buy Off the Shelf."
//
// Teaser-only email body that pushes readers back to the gated page on
// missionmeetstech.com. Full intel lives at
// /premium/briefs/capture-corner-2026-06-12.html.
//
// Schedule: 0 13 * * * (13:00 UTC = 09:00 ET) via netlify.toml. Also
// HTTP-invocable for an immediate manual run (handler ignores the event).
// One-shot guards:
//   1. DATE GUARD — fires only on/after 2026-06-12 UTC.
//   2. IDEMPOTENCY GUARD — checks ops_events for a prior
//      `jun12_expert_test_capture_sent` event; exits if already sent.
//   3. KILL SWITCH — env var JUN12_EXPERT_TEST_DISABLED=true halts.
//
// Reads `data/jun-12-release/capture-corner-teaser.md` from the function
// bundle (registered in netlify.toml included_files). Renders markdown to
// HTML, sends per-subscriber via Resend, logs every send to ops_events.
//
// Retire pattern: delete this function + its data/jun-12-release/**
// included_files entry + the netlify.toml cron block within ~14 days of
// the run date (target: by 2026-06-26).
// ============================================================
...
const SUBJECT = "[MMT Premium] The Expert Test — Capture Corner live on the site";
const RUN_DATE_UTC = "2026-06-12";
const BRIEF_URL = "https://missionmeetstech.com/premium/briefs/capture-corner-2026-06-12.html";
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "jun-12-release", "capture-corner-teaser.md");
const FALLBACK_PATH = path.join(__dirname, "data", "jun-12-release", "capture-corner-teaser.md");
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "jun12_expert_test_capture_sent")
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      ...
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">Capture Corner &middot; June 12, 2026</div>
    </div>
...
exports.handler = async () => {
  ...
  console.log("jun12-expert-test-capture-send: triggered", checkedAt);

  if (String(process.env.JUN12_EXPERT_TEST_DISABLED || "").toLowerCase() === "true") {
  ...
  if (await alreadySent(supabase)) {
    console.log("jun12-expert-test-capture-send: already sent, skipping");
    return { statusCode: 200, body: JSON.stringify({ skipped: "already_sent", checkedAt }) };
  }

  const bodyMd = readBodyMd();
  if (!bodyMd) {
    console.error("jun12-expert-test-capture-send: source markdown missing");
  ...
  if (subErr) {
    console.error("jun12-expert-test-capture-send: subscriber query failed:", subErr.message);
  ...
  console.log(`jun12-expert-test-capture-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`jun12-expert-test-capture-send: send to ${s.email} failed:`, err.message);
    }
  }

  try {
    const { error: logErr } = await supabase.from("ops_events").insert({
      event_type: "jun12_expert_test_capture_sent",
      source_function: "jun12-expert-test-capture-send",
      details: { sent_count: sent, failed_count: failed, eligible_count: paid.length, checked_at: checkedAt, failures: failures.slice(0, 10) },
    });
    if (logErr) console.warn("jun12-expert-test-capture-send: ops_events log failed:", logErr.message);
  } catch (logErr) {
    console.warn("jun12-expert-test-capture-send: ops_events log failed:", logErr.message);
  }
