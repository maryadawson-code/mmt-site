...
// lib/federal-data-apis.js saves the answers for the UTC day.
//
// Also finishes one subscriber question whose award search timed out
// (lib/usaspending-handoff.js POSTs { questions: [question] }), so the
// retry is answered from the saved copy.
//
...
const { warmUSASpending } = require("./lib/federal-data-apis");
...
async function warmAll({ questions = VEHICLES.map((v) => v.canonical), warm = warmUSASpending, queryFor = federalQueryFor, ms = NIGHTLY_MS } = {}) {
  ...
  const failed = results.filter((r) => !/^(fetched|cached)$/.test(String(r.awards)));
...
exports.handler = async (event) => {
  ...
  console.log(`[usaspending-prewarm] ${mode}: ${summary.queries} queries, ${summary.failed} not warmed`, JSON.stringify(summary.results));
