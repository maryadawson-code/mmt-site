// ============================================================
// may8-premium-capture-send.js — Netlify Scheduled Function
//
// Sends the May 8, 2026 Capture Corner (COMP/PSCP — Vendors,
// Vehicles, and the Mandatory Floor That Lives in One Subaccount)
// to all active premium subscribers.
//
// Schedule: daily at 10:00 UTC (06:00 ET) via netlify.toml.
// One-shot guards inside the function:
//   1. DATE GUARD — fires only when today is 2026-05-08 (UTC).
//   2. IDEMPOTENCY GUARD — checks ops_events for a prior
//      `may8_premium_capture_sent` event; exits if already sent.
//   3. KILL SWITCH — env var MAY8_PREMIUM_CAPTURE_DISABLED=true halts.
//
// Reads `data/may-8-release/capture-corner-premium.md` from the
...
const SUBJECT = "[MMT Premium] Capture Corner: COMP/PSCP — Vendors, Vehicles, and the Mandatory Floor in One Subaccount";
const RUN_DATE_UTC = "2026-05-08";
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "may-8-release", "capture-corner-premium.md");
const FALLBACK_PATH = path.join(__dirname, "data", "may-8-release", "capture-corner-premium.md");
const CHART_URL = "https://missionmeetstech.com/premium/briefs/charts/comp_pscp_chart.png";
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "may8_premium_capture_sent")
...
function renderEmailHtml(bodyMd) {
  ...
  let md = bodyMd.replace(/^# .+?\n/, "").trim();
  // Rewrite the chart reference (charts/comp_pscp_chart.png) to the live CDN URL.
  md = md.replace(/charts\/comp_pscp_chart\.png/g, CHART_URL);
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      ...
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">Capture Corner · May 8, 2026</div>
    </div>
...
exports.handler = async () => {
  ...
  console.log("may8-premium-capture-send: triggered", checkedAt);

  if (String(process.env.MAY8_PREMIUM_CAPTURE_DISABLED || "").toLowerCase() === "true") {
  ...
  if (await alreadySent(supabase)) {
    console.log("may8-premium-capture-send: already sent, skipping");
    return { statusCode: 200, body: JSON.stringify({ skipped: "already_sent", checkedAt }) };
  }

  const bodyMd = readBodyMd();
  if (!bodyMd) {
    console.error("may8-premium-capture-send: source markdown missing");
  ...
  if (subErr) {
    console.error("may8-premium-capture-send: subscriber query failed:", subErr.message);
  ...
  console.log(`may8-premium-capture-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`may8-premium-capture-send: send to ${s.email} failed:`, err.message);
    }
  }

  try {
    await supabase.from("ops_events").insert({
      event_type: "may8_premium_capture_sent",
      details: { sent_count: sent, failed_count: failed, eligible_count: paid.length, checked_at: checkedAt, failures: failures.slice(0, 10) },
    });
  } catch (logErr) {
    console.warn("may8-premium-capture-send: ops_events log failed:", logErr.message);
  }
