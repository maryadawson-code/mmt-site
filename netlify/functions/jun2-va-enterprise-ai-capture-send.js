// ============================================================
// jun2-va-enterprise-ai-capture-send.js — Netlify Scheduled Function
//
// Sends the June 2, 2026 Capture Corner teaser ("VA Enterprise AI")
// to all active premium subscribers. Premium companion to the public
// newsletter "Industry Leader, Out of Scope."
//
// Teaser-only email body that pushes readers back to the gated page on
// missionmeetstech.com. Full intel lives at
// /premium/briefs/capture-corner-2026-06-02.html.
//
// Schedule: 0 13 * * * (13:00 UTC = 09:00 ET) via netlify.toml.
// One-shot guards:
//   1. DATE GUARD — fires only on/after 2026-06-02 UTC.
//   2. IDEMPOTENCY GUARD — checks ops_events for a prior
//      `jun2_va_enterprise_ai_capture_sent` event; exits if already sent.
//   3. KILL SWITCH — env var JUN2_VA_ENTERPRISE_AI_DISABLED=true halts.
//
// Reads `data/jun-2-release/capture-corner-teaser.md` from the
...
//
// Retire pattern: delete this function + its data/jun-2-release/**
// included_files entry + the netlify.toml cron block within ~14 days
// of the run date (target: by 2026-06-16).
// ============================================================
...
const SUBJECT = "[MMT Premium] VA enterprise AI — Capture Corner live on the site";
const RUN_DATE_UTC = "2026-06-02";
const BRIEF_URL = "https://missionmeetstech.com/premium/briefs/capture-corner-2026-06-02.html";
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "jun-2-release", "capture-corner-teaser.md");
const FALLBACK_PATH = path.join(__dirname, "data", "jun-2-release", "capture-corner-teaser.md");
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "jun2_va_enterprise_ai_capture_sent")
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      ...
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">Capture Corner &middot; June 2, 2026</div>
    </div>
...
exports.handler = async () => {
  ...
  console.log("jun2-va-enterprise-ai-capture-send: triggered", checkedAt);

  if (String(process.env.JUN2_VA_ENTERPRISE_AI_DISABLED || "").toLowerCase() === "true") {
  ...
  if (await alreadySent(supabase)) {
    console.log("jun2-va-enterprise-ai-capture-send: already sent, skipping");
    return { statusCode: 200, body: JSON.stringify({ skipped: "already_sent", checkedAt }) };
  }

  const bodyMd = readBodyMd();
  if (!bodyMd) {
    console.error("jun2-va-enterprise-ai-capture-send: source markdown missing");
  ...
  if (subErr) {
    console.error("jun2-va-enterprise-ai-capture-send: subscriber query failed:", subErr.message);
  ...
  console.log(`jun2-va-enterprise-ai-capture-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`jun2-va-enterprise-ai-capture-send: send to ${s.email} failed:`, err.message);
    }
  }

  try {
    await supabase.from("ops_events").insert({
      event_type: "jun2_va_enterprise_ai_capture_sent",
      details: { sent_count: sent, failed_count: failed, eligible_count: paid.length, checked_at: checkedAt, failures: failures.slice(0, 10) },
    });
  } catch (logErr) {
    console.warn("jun2-va-enterprise-ai-capture-send: ops_events log failed:", logErr.message);
  }
