...
const NAICS_CODES = ["541512", "541511", "541519", "541611", "621999", "334510"];
...
async function classifyWithClaude(awards) {
  ...
  Agency: ${a["Awarding Agency"] || "N/A"} / ${a["Awarding Sub Agency"] || "N/A"}
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `You are a federal procurement analyst specializing in health IT contract vehicles.

Below are ${awards.length} recent federal health IT contract awards to small businesses. Classify each under ONE of these contract vehicles:
...
- Not health-related = below 40 (exclude these)
...
exports.handler = async (event) => {
  ...
  // --- Fetch 1: Health IT awards to small businesses at key agencies ---
  console.log("Fetching health IT small business awards from key agencies...");
  try {
    ...
    console.log(`  Key agencies: ${results.length} awards`);
  } catch (err) {
    console.error("Fetch 1 failed:", err.message);
  }

  // --- Fetch 2: All federal health IT awards to small businesses ---
  console.log("Fetching all federal health IT small business awards...");
  try {
    const results = await fetchAwards({
    ...
    allResults.push(...results);
    console.log(`  All agencies: ${results.length} awards`);
  } catch (err) {
    console.error("Fetch 2 failed:", err.message);
  }

  // --- Fetch 3: Health-related keywords for broader coverage ---
  console.log("Fetching keyword-matched health awards...");
  try {
    const results = await fetchAwards({
      keywords: ["electronic health record", "telehealth", "health IT"],
      time_period: [{ start_date: startDate, end_date: endDate }],
      award_type_codes: ["A", "B", "C", "D"],
      recipient_type_names: ["small_business"],
    });
    allResults.push(...results);
    console.log(`  Keyword search: ${results.length} awards`);
  } catch (err) {
  ...
  for (const r of allResults) {
    ...
    if (id && !seenIds.has(id)) {
      seenIds.add(id);
      // Only include awards with meaningful amounts
  ...
  // Sort by amount descending and take top 40
  ...
  if (topResults.length === 0) {
    console.log("No awards to classify");
    return { statusCode: 200, body: JSON.stringify({ fetched: 0, classified: 0, upserted: 0 }) };
  }
  ...
  for (const cls of classifications) {
    ...
    const vehicle = VALID_VEHICLES.includes(cls.contract_vehicle) ? cls.contract_vehicle : null;
    if (!vehicle) continue;
