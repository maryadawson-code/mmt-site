// ============================================================
// apr29-dha-reorg-update-send.js — Netlify Scheduled Function
//
// Sends the v2 UPDATE notice for the April 29, 2026 DHA Reorg
// premium piece. v1 went out this morning via apr29-dha-reorg-send.
// After the AD-RDA Town Hall slides landed, the article was revised
// (PEO DHMS matrix, OWHA filled by RDML Farrill, three named PAEs,
// new governance section, July 19 FOC). This sends a short update
// email pointing premium readers at the revised article.
//
// Schedule: every 15 minutes via netlify.toml; auto-stops via guards.
// One-shot guards inside the function:
//   1. WINDOW GUARD — fires only on 2026-04-29 or 2026-04-30 (UTC).
//   2. IDEMPOTENCY GUARD — checks ops_events for prior
//      `apr29_dha_reorg_update_sent` event; exits if already sent.
//   3. KILL SWITCH — env var APR29_DHA_REORG_UPDATE_DISABLED=true halts.
//
// Reads `data/apr-29-release/dha-reorg-update.md` from the function
// bundle (registered in netlify.toml included_files). Renders markdown
// to HTML, sends per-subscriber via Resend, logs every send.
// ============================================================
...
const SUBJECT = "[MMT Premium] Update: DHA reorg brief revised with today's Town Hall intel";
const RUN_DATES_UTC = new Set(["2026-04-29", "2026-04-30"]);
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "apr-29-release", "dha-reorg-update.md");
const FALLBACK_PATH = path.join(__dirname, "data", "apr-29-release", "dha-reorg-update.md");

function withinRunWindow() {
  return RUN_DATES_UTC.has(new Date().toISOString().slice(0, 10));
}
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "apr29_dha_reorg_update_sent")
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      <span style="font-size:18px;font-weight:800;color:#FFFFFF;">★ Mission Meets Tech &middot; Premium</span>
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">DHA Reorganization &middot; Update &middot; April 29, 2026</div>
    </div>
...
exports.handler = async () => {
  ...
  console.log("apr29-dha-reorg-update-send: triggered", checkedAt);

  if (String(process.env.APR29_DHA_REORG_UPDATE_DISABLED || "").toLowerCase() === "true") {
    return { statusCode: 200, body: JSON.stringify({ skipped: "kill_switch", checkedAt }) };
  }
  if (!withinRunWindow()) {
    return { statusCode: 200, body: JSON.stringify({ skipped: "outside_run_window", today: new Date().toISOString().slice(0, 10), expected: [...RUN_DATES_UTC] }) };
  }
  ...
  if (await alreadySent(supabase)) {
    console.log("apr29-dha-reorg-update-send: already sent, skipping");
    return { statusCode: 200, body: JSON.stringify({ skipped: "already_sent", checkedAt }) };
  }

  const bodyMd = readBodyMd();
  if (!bodyMd) {
    console.error("apr29-dha-reorg-update-send: source markdown missing");
  ...
  // Same eligibility model as apr29-dha-reorg-send (v1) so the audience matches.
  ...
  if (subErr) {
    console.error("apr29-dha-reorg-update-send: subscriber query failed:", subErr.message);
  ...
  console.log(`apr29-dha-reorg-update-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`apr29-dha-reorg-update-send: send to ${s.email} failed:`, err.message);
    }
  }

  try {
    await supabase.from("ops_events").insert({
      event_type: "apr29_dha_reorg_update_sent",
      details: { sent_count: sent, failed_count: failed, eligible_count: paid.length, checked_at: checkedAt, failures: failures.slice(0, 10) },
    });
  } catch (logErr) {
    console.warn("apr29-dha-reorg-update-send: ops_events log failed:", logErr.message);
  }
