// ============================================================
// may12-premium-capture-send.js — Netlify Scheduled Function
//
// Sends the May 12, 2026 Capture Corner ("Where the Pendulum
// Becomes Spend") to all active premium subscribers. Paid
// companion to the May 12 public article "The Warfighter Under
// the Pendulum."
//
// Schedule: daily at 10:00 UTC (06:00 ET) via netlify.toml.
// One-shot guards inside the function:
//   1. DATE GUARD — fires only when today is 2026-05-12 (UTC).
//   2. IDEMPOTENCY GUARD — checks ops_events for a prior
//      `may12_premium_capture_sent` event; exits if already sent.
//   3. KILL SWITCH — env var MAY12_PREMIUM_CAPTURE_DISABLED=true halts.
//
// Reads `data/may-12-release/capture-corner-premium.md` from the
...
const SUBJECT = "[MMT Premium] Capture Corner: Where the Pendulum Becomes Spend";
const RUN_DATE_UTC = "2026-05-12";
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "may-12-release", "capture-corner-premium.md");
const FALLBACK_PATH = path.join(__dirname, "data", "may-12-release", "capture-corner-premium.md");
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "may12_premium_capture_sent")
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      ...
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">Capture Corner · May 12, 2026</div>
    </div>
...
exports.handler = async () => {
  ...
  console.log("may12-premium-capture-send: triggered", checkedAt);

  if (String(process.env.MAY12_PREMIUM_CAPTURE_DISABLED || "").toLowerCase() === "true") {
  ...
  if (await alreadySent(supabase)) {
    console.log("may12-premium-capture-send: already sent, skipping");
    return { statusCode: 200, body: JSON.stringify({ skipped: "already_sent", checkedAt }) };
  }

  const bodyMd = readBodyMd();
  if (!bodyMd) {
    console.error("may12-premium-capture-send: source markdown missing");
  ...
  if (subErr) {
    console.error("may12-premium-capture-send: subscriber query failed:", subErr.message);
  ...
  console.log(`may12-premium-capture-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`may12-premium-capture-send: send to ${s.email} failed:`, err.message);
    }
  }

  try {
    await supabase.from("ops_events").insert({
      event_type: "may12_premium_capture_sent",
      details: { sent_count: sent, failed_count: failed, eligible_count: paid.length, checked_at: checkedAt, failures: failures.slice(0, 10) },
    });
  } catch (logErr) {
    console.warn("may12-premium-capture-send: ops_events log failed:", logErr.message);
  }
