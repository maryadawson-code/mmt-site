// ============================================================
// apr29-dha-reorg-send.js — Netlify Scheduled Function
//
// Sends the April 29, 2026 DHA Reorganization premium piece to all
// active premium subscribers, paired with the LinkedIn cadence
// (Mary handles the LinkedIn post and interactions).
//
// Schedule: daily at 13:00 UTC (09:00 ET) via netlify.toml.
// One-shot guards inside the function:
//   1. DATE GUARD — fires only when today is 2026-04-29 (UTC).
//   2. IDEMPOTENCY GUARD — checks ops_events for prior
//      `apr29_dha_reorg_sent` event; exits if already sent.
//   3. KILL SWITCH — env var APR29_DHA_REORG_DISABLED=true halts.
//
// Reads `data/apr-29-release/dha-reorg-premium.md` from the function
...
const SUBJECT = "[MMT Premium] DHA Reorganization: Sixteen Changes, Five That Move Your Pipeline";
const RUN_DATE_UTC = "2026-04-29";
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "apr-29-release", "dha-reorg-premium.md");
const FALLBACK_PATH = path.join(__dirname, "data", "apr-29-release", "dha-reorg-premium.md");
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "apr29_dha_reorg_sent")
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      ...
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">DHA Reorganization &middot; April 29, 2026</div>
    </div>
...
exports.handler = async () => {
  ...
  console.log("apr29-dha-reorg-send: triggered", checkedAt);

  if (String(process.env.APR29_DHA_REORG_DISABLED || "").toLowerCase() === "true") {
  ...
  if (await alreadySent(supabase)) {
    console.log("apr29-dha-reorg-send: already sent, skipping");
    return { statusCode: 200, body: JSON.stringify({ skipped: "already_sent", checkedAt }) };
  }

  const bodyMd = readBodyMd();
  if (!bodyMd) {
    console.error("apr29-dha-reorg-send: source markdown missing");
  ...
  // Premium subscribers — same eligibility model as may1-premium-capture-send.
  ...
  if (subErr) {
    console.error("apr29-dha-reorg-send: subscriber query failed:", subErr.message);
  ...
  console.log(`apr29-dha-reorg-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`apr29-dha-reorg-send: send to ${s.email} failed:`, err.message);
    }
  }

  try {
    await supabase.from("ops_events").insert({
      event_type: "apr29_dha_reorg_sent",
      details: { sent_count: sent, failed_count: failed, eligible_count: paid.length, checked_at: checkedAt, failures: failures.slice(0, 10) },
    });
  } catch (logErr) {
    console.warn("apr29-dha-reorg-send: ops_events log failed:", logErr.message);
  }
