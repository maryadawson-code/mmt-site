// ============================================================
// may19-fraud-workflow-send.js — Netlify Scheduled Function
//
// Sends the May 19, 2026 Capture Corner ("The Program-Integrity
// Market Just Moved Upstream") to all active premium subscribers.
// Premium companion to the public Tuesday article "The Fraud Was
// in the Workflow."
//
...
// inside the function:
//   1. DATE GUARD — fires only on/after 2026-05-19 UTC.
//   2. IDEMPOTENCY GUARD — checks ops_events for a prior
//      `may19_fraud_workflow_sent` event; exits if already sent.
//   3. KILL SWITCH — env var MAY19_FRAUD_WORKFLOW_DISABLED=true halts.
//
// Reads `data/may-19-release/capture-corner-premium.md` from the
...
//
// Retire pattern: delete this function + its data/may-19-release/**
// included_files entry + the netlify.toml cron block within ~14 days
// of the run date (target: by 2026-06-02).
// ============================================================
...
const SUBJECT = "[MMT Premium] The program-integrity market just moved upstream — 6 capture lanes + the May 13 moratorium translated";
const RUN_DATE_UTC = "2026-05-19";
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "may-19-release", "capture-corner-premium.md");
const FALLBACK_PATH = path.join(__dirname, "data", "may-19-release", "capture-corner-premium.md");
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "may19_fraud_workflow_sent")
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      ...
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">Capture Corner &middot; May 19, 2026</div>
      <div style="font-size:20px;font-weight:800;margin-top:14px;line-height:1.25;">The Program-Integrity Market Just Moved Upstream</div>
      <div style="font-size:13px;color:#9ec3e6;margin-top:4px;">Six capture lanes. The May 13 nationwide HHA/Hospice moratorium translated into procurement surfaces. The SMRC recompete window. DOJ's strike force posture. The buyer map.</div>
    </div>
    <div style="padding:32px;">
      ...
      <div style="margin-top:48px;padding:28px 24px;border:2px solid #92710A;border-radius:12px;background:linear-gradient(180deg,#FFFFFF 0%,#FEF9E7 100%);">
        ...
        <p style="font-size:14px;color:#5C6B7A;margin:0 0 14px;">This Capture Corner surfaces the architecture and the six lanes. Where it stops short &mdash; named decision makers at CMS CPI, the SMRC prime field, your firm's specific positioning against the six WISeR participants, the teaming map for the West Coast Strike Force surface area, the OIG referral package your offering would actually produce &mdash; the next layer of intelligence is custom by request.</p>
        ...
        <p style="margin:0 0 18px;"><a href="mailto:mary@missionmeetstech.com?subject=Program%20Integrity%20Deep%20Dive%20Request&amp;body=Hi%20Mary%2C%0A%0AI%27d%20like%20to%20request%20a%20%2450%20custom%20deep%20dive%20on%3A%0A%0A%5BYour%20topic%20here%5D%0A%0AKey%20question%20I%20need%20answered%3A%0A%0A%5BYour%20question%20here%5D%0A%0AHow%20I%27ll%20use%20the%20memo%3A%0A%0A%5BPipeline%20review%2C%20capture%20kickoff%2C%20teaming%20decision%2C%20etc.%5D%0A%0AThanks%2C" style="display:inline-block;padding:12px 24px;background:#92710A;color:#FFFFFF;font-weight:700;font-size:14px;border-radius:8px;text-decoration:none;letter-spacing:0.02em;">Request a custom deep dive &rarr;</a></p>
        <p style="font-size:12px;color:#5C6B7A;margin:0;font-style:italic;">Examples: "SMRC prime field forecast" &middot; "Our positioning against the six WISeR participants" &middot; "West Coast Strike Force teaming map" &middot; "OIG referral package architecture"</p>
      </div>
    </div>
...
exports.handler = async () => {
  ...
  console.log("may19-fraud-workflow-send: triggered", checkedAt);

  if (String(process.env.MAY19_FRAUD_WORKFLOW_DISABLED || "").toLowerCase() === "true") {
  ...
  if (await alreadySent(supabase)) {
    console.log("may19-fraud-workflow-send: already sent, skipping");
    return { statusCode: 200, body: JSON.stringify({ skipped: "already_sent", checkedAt }) };
  }

  const bodyMd = readBodyMd();
  if (!bodyMd) {
    console.error("may19-fraud-workflow-send: source markdown missing");
  ...
  // Same paid-subscriber query as may15 / may17.
  ...
  if (subErr) {
    console.error("may19-fraud-workflow-send: subscriber query failed:", subErr.message);
  ...
  console.log(`may19-fraud-workflow-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`may19-fraud-workflow-send: send to ${s.email} failed:`, err.message);
    }
  }

  try {
    await supabase.from("ops_events").insert({
      event_type: "may19_fraud_workflow_sent",
      details: { sent_count: sent, failed_count: failed, eligible_count: paid.length, checked_at: checkedAt, failures: failures.slice(0, 10) },
    });
  } catch (logErr) {
    console.warn("may19-fraud-workflow-send: ops_events log failed:", logErr.message);
  }
