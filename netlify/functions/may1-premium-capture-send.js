// ============================================================
// may1-premium-capture-send.js — Netlify Scheduled Function
//
// Sends the May 1, 2026 Premium Capture Intelligence Sheet
// (VA Enterprise Imaging) to all active premium subscribers.
//
...
//      `may1_premium_capture_sent` event; exits if already sent.
//   3. KILL SWITCH — env var MAY1_PREMIUM_CAPTURE_DISABLED=true halts.
//
// Reads `data/may-1-release/premium-capture-sheet.md` from the
...
//
// Spec: docs/may-1-release-runbook.md (06:00 ET row).
// ============================================================
...
const SUBJECT = "[MMT Premium] May Capture Intelligence: VA Enterprise Imaging";
const RUN_DATE_UTC = "2026-05-01"; // ISO YYYY-MM-DD; UTC
const SOURCE_PATH = path.join(__dirname, "..", "..", "data", "may-1-release", "premium-capture-sheet.md");
const FALLBACK_PATH = path.join(__dirname, "data", "may-1-release", "premium-capture-sheet.md");
...
async function alreadySent(supabase) {
  const { data } = await supabase
    ...
    .eq("event_type", "may1_premium_capture_sent")
...
<body style="margin:0;padding:0;background:#FFFFFF;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;color:#0A192F;line-height:1.6;">
  <div style="max-width:680px;margin:0 auto;background:#FFFFFF;">
    <div style="background:#0A192F;padding:24px 32px;color:#FFFFFF;">
      ...
      <div style="font-size:12px;color:#9ec3e6;margin-top:4px;letter-spacing:0.06em;text-transform:uppercase;">Capture Intelligence Sheet · May 2026</div>
    </div>
...
exports.handler = async (event) => {
  ...
  console.log("may1-premium-capture-send: triggered", checkedAt);

  // Kill switch
  if (String(process.env.MAY1_PREMIUM_CAPTURE_DISABLED || "").toLowerCase() === "true") {
    return { statusCode: 200, body: JSON.stringify({ skipped: "kill_switch", checkedAt }) };
  }

  // Date guard — fire only on 2026-05-01 (UTC).
  if (!todayIsRunDate()) {
  ...
  if (await alreadySent(supabase)) {
    console.log("may1-premium-capture-send: already sent, skipping");
  ...
  if (!bodyMd) {
    console.error("may1-premium-capture-send: source markdown missing");
  ...
  if (subErr) {
    console.error("may1-premium-capture-send: subscriber query failed:", subErr.message);
  ...
  console.log(`may1-premium-capture-send: ${paid.length} eligible subscribers`);
  ...
  for (const s of paid) {
    try {
      ...
      console.warn(`may1-premium-capture-send: send to ${s.email} failed:`, err.message);
    }
  }
