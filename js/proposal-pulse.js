...
async function submitDeck() {
  ...
  try {
    ...
    startPolling(scoringId, gatewayData);
...
function startPolling(scoringId, gatewayData) {
  stopPolling();
  ...
  pollTimer = setInterval(async () => {
    try {
      const response = await fetch(STATUS_URL + '?scoring_id=' + encodeURIComponent(scoringId));
