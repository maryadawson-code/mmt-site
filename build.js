...
function injectDashShell(html, activePage) {
  ...
  const links = [
    ...
    { href: '/premium/market-entry/', label: 'Market Entry', id: 'market-entry' },
    { href: '/premium/state-medicaid/', label: 'State Medicaid', id: 'state-medicaid' },
    { href: '/premium/dhits-2026/', label: 'DHITS 2026 Slides', id: 'dhits-2026' },
...
<footer class="wrap"></footer>
  ...
  const dashPageMap = {
    ...
    // DHITS 2026 slide library (2026-09-15): generated by
    // scripts/generate-dhits-slide-library.js from data/dhits-2026-slides.json
    'premium/dhits-2026.html': 'dhits-2026',
    // Market-entry coverage (2026-09-20): reference pages rendered client-side
    // from data/reference/*.json (docs/market-entry-coverage-spec.md).
    'premium/market-entry.html': 'market-entry',
    'premium/state-medicaid.html': 'state-medicaid',
  };
  ...
  const subDirPages = [
    ...
    { src: 'premium/dhits-2026.html', dest: 'premium/dhits-2026/index.html' },
    { src: 'premium/market-entry.html', dest: 'premium/market-entry/index.html' },
    { src: 'premium/state-medicaid.html', dest: 'premium/state-medicaid/index.html' },
  ...
  if (fs.existsSync(dataSrcDir)) {
    ...
    // Market-entry coverage (2026-09-20): data/reference/*.json is the one
    // subdirectory that IS a public asset. premium/market-entry.html and
    // premium/state-medicaid.html fetch it client-side, the same way
    // key-people.html reads /data/key-people.json.
    const refSrcDir = path.join(dataSrcDir, 'reference');
    if (fs.existsSync(refSrcDir)) {
      const refDistDir = path.join(dataDistDir, 'reference');
      ensureDir(refDistDir);
      let refCopied = 0;
      for (const f of fs.readdirSync(refSrcDir)) {
        if (!f.endsWith('.json')) continue;
        fs.copyFileSync(path.join(refSrcDir, f), path.join(refDistDir, f));
        refCopied++;
      }
      console.log(`Copied ${refCopied} data/reference/*.json file(s) to dist/data/reference/`);
    }
