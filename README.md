...
3. Go to **Settings** → **Integrations** (or **Webhooks**)
...
3. Click on each form → **Form notifications** → **Add notification**
4. Choose **Email notification** and enter your email
5. Now you'll get an email whenever someone submits!

**View all submissions:** Netlify dashboard → **Forms** tab
...
├── build.js             # Fetches RSS, generates podcast page
...
└── src/                 # Source files
    ...
    ├── contact.html     # Contact form + email signup
    ├── newsletter.html  # Newsletter overview
    ├── newsletter-archive.html  # All past issues
    ├── podcast.template.html    # Podcast page (auto-updated)
    ├── resources.html   # Quick reference guides
    ├── styles.css       # All styles
    ├── favicon.svg      # Browser tab icon
    ├── data/
    │   └── newsletters.json     # Newsletter archive data (YOU EDIT THIS)
    └── images/
        ├── mary-womack.jpg
        ├── sara-byrd.jpg
        ├── og-default.svg       # Social share images
        └── og-podcast.svg
```
...
4. Push to GitHub → site rebuilds with new archive entry
...
4. Transistor webhook → Netlify rebuilds → podcast page updates automatically
